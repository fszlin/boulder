...
def test_revoke_by_account():
    ...
    verify_ocsp(cert_file.name, "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002", "revoked")
    verify_akamai_purge()
...
def test_ocsp():
    ...
    verify_ocsp(cert_file.name, "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002", "good")
...
def ocsp_exp_unauth_setup():
    ...
    check_ocsp_basic_oid(cert_file.name, "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002")
...
def test_ocsp_exp_unauth():
    ...
    while tries < 5:
        try:
            verify_ocsp(cert_file, "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002", "XXX")
...
def ocsp_resigning_setup():
    ...
    ocsp_response, reason = get_ocsp_response_and_reason(
        cert_file.name, "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002")
...
def test_ocsp_resigning():
    ...
    while tries < 5:
        resp, reason = get_ocsp_response_and_reason(
            ocsp_resigning_setup_data['cert_file'], "certes/intermediate-cert-rsa-a.pem", "http://localhost:4002")
